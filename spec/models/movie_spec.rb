...
describe Movie do
  describe 'searching Tmdb by keyword' do
    context 'with valid key' do
      ...
      context 'with matches' do
        it "should render an array of hashes with two movies and if no rating then add NA" do
          ...
          expect(results[0][:rating]).to eq('NA')
  ...
  context 'with valid movie' do
    it 'should add the movie to the database if no rating then add NA' do
      ...
      expect(results["rating"]).to eq("NA")
    end  
  end  
end
